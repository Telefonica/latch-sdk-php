<?php namespace ElevenPaths\Latch;
...
final class Latch {
    ...
    private function httpGetProxy($url) {
        return new Response($this->httpGet(self::$API_HOST . $url, $this->authenticationHeaders("GET", $url, null)));
    }
    
    public function pairWithId($accountId) {
        return $this->httpGetProxy(self::$API_PAIR_WITH_ID_URL . "/" . $accountId);
    }
    
    public function pair($token) {
        return $this->httpGetProxy(self::$API_PAIR_URL . "/" . $token);
    }
    
    public function status($accountId) {
        return $this->httpGetProxy(self::$API_CHECK_STATUS_URL . "/" . $accountId);
    }

    public function operationStatus($accountId, $operationId) {
        return $this->httpGetProxy(self::$API_CHECK_STATUS_URL . "/" . $accountId . "/op/" . $operationId);
    }
        
    public function unpair($accountId) {
        return $this->httpGetProxy(self::$API_UNPAIR_URL . "/" . $accountId);
    }
